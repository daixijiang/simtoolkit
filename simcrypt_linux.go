...
int vsim_encrypt(SRC_SIM_DATA *profile, ENC_SIM_DATA *encProfile)
{
	void* handle;
	typedef int (*FPTR)(SRC_SIM_DATA *, ENC_SIM_DATA *);

	handle = dlopen("./libsimcrypt.so", 1);
	if (!handle) {
		return -1;
	}

	FPTR fptr = (FPTR)dlsym(handle, "processProfileData");
	if (!fptr) {
		return -2;
	}

	int result = (*fptr)(profile, encProfile);
	return result;
}
